...
- **A Vite CSP `unsafe-eval` warning in DevTools is noise, not a bug**: Vite uses `eval` in development mode for fast HMR source maps. It's expected, it doesn't affect the app, and it won't appear in production. It can be suppressed by switching to inline source maps in `vite.config.js` (`esbuild: { sourcemap: 'inline' }`).
