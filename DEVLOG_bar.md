...
- **Derived state vs stored state**: `total` is computed on the fly from `items` (`items.reduce(...)`) rather than stored as its own field in state. If it were stored, you'd have to remember to update it every time items change — a source of bugs. Derived values that can be computed from existing state should always be computed, never stored.
